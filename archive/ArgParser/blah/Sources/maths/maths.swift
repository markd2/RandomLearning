...
struct Math: ParsableCommand {
    static let configuration = CommandConfiguration(
      ...
      version: "1.0.0",
      // set up a nested tree of subcommands with language support.
      // for type-level introspection, this could be provided
      // by automatically finding nested `ParsableCommand` types.
...
extension Math {
    ...
    struct Add: ParsableCommand {
        static let configuration =
          CommandConfiguration(abstract: "Print the sum of values.")
...
extension Math {
    struct Statistics: ParsableCommand {
        static let configuration = CommandConfiguration(
          commandName: "stats", // vs the inferred name from the type
...
extension Math.Statistics {
    ...
    struct Quantiles: ParsableCommand {
        ...
        @Argument(completion: .list(["alphabet", "alligator", "branch",
                                     "braggart"]))
        var oneOfFour: String?
        
        @Argument(completion: .custom { _ in ["alabaster", "breakfast",
                                              "crunch", "crash"] })
