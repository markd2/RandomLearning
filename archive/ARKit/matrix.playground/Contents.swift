...
let scaleDownAction = SCNAction.scale(to: 0.5, duration: scaleActionDuration)
let scaleUpAction = SCNAction.scale(to: 1.5, duration: scaleActionDuration)
...
cubeNode.runAction(scaleSequenceAction) {
    print("Cube node scaline:", cubeNode.simdScale)
    print("Cube node transform:", cubeNode.simdTransform)
}
